// Reviewer 4, comment 7, on "When only the asserted graph is validated, the shape is not
// applied": "I am not sure what is meant here. Do the authors mean that the graph does not
// satisfy the shape?"
//
// It did not mean that, and the report could not tell you which was meant. `conforms: true`
// covers both "every selected focus node satisfied the shape" and "the shape selected
// nothing, so nothing was checked". Those are very different claims about a dataset, and
// the difference is the worked example: before classification the semiconductor shape
// selects no focus node, after it selects one and a missing quality becomes visible.
//
// runShaclValidation now reports focus-node counts per shape, so the distinction is visible
