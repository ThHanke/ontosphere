// Reasoner calls must not terminate each other.
//
// rdf-reasoner-konclude's terminate() rejects every pending call with "Worker terminated", and a
// reasoning run recycles the reasoner when it starts. If two reasoner-using commands overlap, or a
// graph removal resets the reasoner during a run, the in-flight call dies. Observed in the
// browser as "OWL DL reasoning could not complete: Worker terminated" in 2 of 3 runs of the
// PMDco workflow spec.
//
