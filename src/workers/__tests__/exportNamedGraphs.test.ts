// Register item A-03: the manuscript describes JSON-LD as intrinsically unable to carry
// named graphs. It is not: JSON-LD 1.1 expresses a dataset as node objects with @graph.
// The exporter was flattening every quad into the default graph, so an exported record
// could not be told apart from the statements that had been inferred from it -- exactly
// the asserted/inferred distinction the paper relies on named graphs to preserve.
//
// These tests pin the dataset formats: N-Quads, TriG and JSON-LD must all round-trip the
// urn:vg:* partition, and Turtle (which genuinely cannot) must not pretend to.
