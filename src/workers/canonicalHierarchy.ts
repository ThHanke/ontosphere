...
//
// Konclude reports the inferred named-class hierarchy as a transitive reduction, and the
// reduction it picks can differ between independent sessions on the same input: it emits
// different subsets of the redundant subsumption edges while the transitive closure stays
// identical. Measured with rdf-reasoner-konclude 0.7.1 on PMDco + the Fe-Si record: 50-61
// inferred named subClassOf edges per session, closure 8,246 statements every time.
//
...
// other materialised predicate are passed through untouched.
